...
| `backup/` | **Library backup engine** (item 10; Qt-free package). Dated snapshots to a user-chosen destination *outside* the store → no `.store_version` impact. Layered so a second storage format and a remote backend are additive: `errors` · `options` (value types + `SETTING_*`, leaf) · `scope` (`is_excluded`/`iter_source_files` — a **denylist**: skips `derived/`/`renders/`/`sessions.jsonl`/`assistant/` + `Images/*/siril/` sandboxes, so new authored data is captured without an allowlist to maintain) · `destination` (`store_backup_root`, `supports_hardlinks`, `free_bytes` — no format imports, so both formats build on it) · **`mirrored`** (the shipped format: `<dest>/M110-Backups/<store>/<ts>/` full browsable trees, unchanged files `os.link`ed to the prior snapshot — near-free incrementals; byte-only copy (`.part`+`os.replace`, mtime-preserved) for new/changed files; per-snapshot `.m110-backup-manifest.json` `{rel:{size,mtime,sha256}}` for **`verify`**; atomic `.incomplete`→rename; full-copy fallback where links fail. Identity is parsed from the **dir name**, so anything not timestamp-shaped is skipped — that's what lets another layout share the store root with no flag day) · `retention` (`apply_retention`, count + min-free, oldest-first, never the last one; no age policy by design — it would wipe history after a vacation) · `probe` (**`probe_destination`** → `DestinationInfo`, the pre-flight capability answer for the UI; creates nothing, leaves no probe files) · `schedule` (`options_from_settings`/`due_for_auto_backup` launch check / `due_for_scheduled_backup` hourly-tick daily 02:00, interval as min-age guard). `__init__` is the façade — import `m110.backup` and use the re-exports |
