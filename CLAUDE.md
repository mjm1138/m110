...
| Module | Role |
|---|---|
| `config.py` | data-root resolution, dir bootstrap/seed (incl. `_seed_library` from the active goals + launch-time `goals.ensure_library_has_active_goals` reconcile), per-target path helpers, `GOALS_TOML`/`LIBRARY_TOML` per-store paths, settings persistence, Seestar mount detection (`find_seestar_myworks`) |
| `migrate.py` | in-place, idempotent, version-stamped migration of an older store to the two-axis layout (`migrate_store`) |
| `catalog.py` | `load_library` (per-store `library.toml` — the user's corpus); `load_reference` (bundled `seed/objects.toml`) + `load_bundled_catalog`/`list_bundled_catalogs` (bundled `seed/catalogs/*.toml` membership) + `catalogs_for_slug` (which catalogs an object is in) + `add_goal_members_to_library` (additively grow the Library from a catalog); `catalog_sort_key`/`season_sort_key`; `load_coords` (reference coords + per-store library `ra_deg/dec_deg`); `object_identifiers`/`object_label` (all designations ordered by catalog hierarchy Messier→Caldwell→NGC/IC) + `catalog_sort_key` (M/C/NGC/IC numeric); `add_captured_objects` (promote captured-but-uncatalogued folders into the Library); `season_from_ra` (derive the observing-season window from RA — calibrated to the curated Messier seasons, ≈98% match; shared with `tools/gen_caldwell.py`); `fill_missing_metadata`/`fill_all_missing_metadata` (backfill an existing Library entry's **missing** fields from the bundled reference + derived season; never overwrites real user values; the right-click / "Library → Fill missing metadata" action) via `_write_library` (in-place rewriter that preserves every key) |
...
| `goals.py` | active-catalog **goals**, stored **per-store** in `.m110_internal_data/goals.toml` (`config.GOALS_TOML`, default Messier — not a global setting, so each store tracks its own and a fresh store starts at the default) — `active_goal_ids` (reads goals.toml) + `set_active_goals` (writes goals.toml + adds the goals' members to the Library via `catalog.add_goal_members_to_library`) + `ensure_library_has_active_goals` (launch-time reconcile so the Library always matches the per-store goals) |
| `seed/` | bundled `objects.toml` (object **reference**: id/type/mag/size/season + J2000 coords; 110 Messier + 109 Caldwell — Caldwell `season` is derived from RA via `tools/gen_caldwell.py`) · `catalogs/<name>.toml` (catalog membership `[members]` slug→designation: Messier, Caldwell) · `priorities.toml` (package-data) |
...
| Module | Role |
|---|---|
| `main.py` | **Shell**: left nav rail (`QListWidget`) → `QStackedWidget` of pages [Summary · Catalog · Processing · Sessions · Journal · Media]; **Summary is the landing page**. Global Ingest (Ctrl+I) toolbar + **M110** menu (Prepare working folders, Preferences Cmd+,) + **Library** menu (Refresh Ctrl+R, Fill missing metadata — bulk reference backfill; room to grow: open/archive/export). `RefreshWorker` (scan→derive→render + `prepare_missing`) drives `page.reload()`; **auto-syncs** on launch / window-focus / after ingest. `open_object(slug)` routes any page's object link → Catalog + selects it. Journal-edit lock disables nav + global actions |
...
| `pages/catalog.py` | the **Library** table (Object/Name/Type/Season/Mag/Size/Filter/Status/Integration/Sessions; Object shows all identifiers via `catalog.object_identifiers`, e.g. "C20 (NGC 7000)") + a **Catalog filter** selector (All / Messier / Caldwell …) + a **"Captured only"** checkbox (default off; composes with catalog + search) + **search** + captured/deep/total **stat row** (per-filter), hosting the shared `DetailPane`; `select_object`/`reload`; per-object import flow; **right-click → "Fill in missing metadata"** (`catalog.fill_missing_metadata`); edit-lock |
