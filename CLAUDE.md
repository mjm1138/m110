...
<data_root>/                         (default ~/Documents/M110)
  ...
  Images/<target>/                   capture-target axis (= the old object_dir)
    ...
    (darks/ flats/ biases/ process/ preserved if present)
...
`m110/seed/` if missing, and writes the internals README — **idempotent, never
overwrites**. Changing the root in Preferences takes effect on **restart**.
