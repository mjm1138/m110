...
| `pages/catalog.py` | the **Library** table (Object/Name/Type/Season/Mag/Size/Filter/Status/Integration/Sessions; Object shows all identifiers via `catalog.object_identifiers`, e.g. "C20 (NGC 7000)") + a **Catalog filter** selector (All / Messier / Caldwell …) + a **"Captured only"** checkbox (default off; composes with catalog + search) + **search** + captured/deep/total **stat row** (per-filter), hosting the shared `DetailPane`; `select_object`/`reload`; per-object import flow; edit-lock |
