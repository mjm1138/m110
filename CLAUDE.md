...
<data_root>/                         (default ~/Documents/M110)
  ...
  .m110_internal_data/               hidden machine state (README: "don't touch")
    ...
    pins.toml           manual Pin/Mute priority overrides (#3): `[pins]` slug→"pin"|"mute"
...
| `pins.py` | **manual Pin/Mute priority overrides** (#3 — the self-contained slice of the ROADMAP-1 auto-prioritizer, shipped ahead of the scorer so the Summary **Priority targets** view has a reason to exist for a fresh user). Per-store `.m110_internal_data/pins.toml` (`config.PINS_TOML`, `[pins]` slug→`"pin"`\|`"mute"`), Qt-free, additive/lazily-created, **survives derived regeneration** (not computed). `load`/`get_state`/`set_state`/`pinned_slugs`/`muted_slugs`. Consumers: `pages/summary.py` (pins surface in Priority targets, mutes excluded, empty-state), Library + Goals right-click with a ▲/▼ marker |
...
| Module | Role |
|---|---|
| `main.py` | **Shell**: left nav rail (`QListWidget`) → `QStackedWidget` of pages [Summary · Goals · Library · Processing · Sessions · Journal · Media]; **Summary is the landing page**. `goals` page's `dirty` → `_do_refresh`. Global Ingest (Ctrl+I) + **Library** menu (Refresh Ctrl+R, Prepare working folders, Add object…, Fill missing metadata — bulk reference backfill, Enrich online… — bulk Simbad enrichment on a worker, Publish, Back up/Restore, Preferences Cmd+, — which folds into the macOS **app menu** by role) + **Help** menu (Report a problem… → `error_report`, About M110). No separate "M110" menubar menu. On macOS the app-menu/dock name is set to "M110" via a best-effort NSBundle patch (`_set_macos_app_name`, needs `pyobjc-framework-Cocoa`; the durable fix is a packaged `.app`). `RefreshWorker` (scan→derive→render + `prepare_missing`) drives `page.reload()`; **auto-syncs** on launch / window-focus / after ingest. `open_object(slug)` routes any page's object link → Catalog + selects it. Summary's `go_to_import` (empty-state CTA) → `_open_ingest`. `main()` calls `logsetup.setup_logging()` + `error_report.install_excepthook(app)` (BETA §5: crashes → dialog+log, not abort) + `first_run_dialog.run_first_run_if_needed()` before the window (first-launch data-folder prompt). `_maybe_backup_nudge` (in `_on_refresh_done`) prompts a backup **once ever**, only once captures exist (`backup_nudge_seen` setting). Journal-edit lock disables nav + global actions |
...
| `pages/catalog.py` | the **Library**: a table view (Object/Name/Type/Season/Mag/Size/Filter/Status/Integration/Sessions; Object shows all identifiers via `catalog.object_identifiers`, e.g. "C20 (NGC 7000)") **and** a **grid view** (`image_grid.TileModel`/`TileDelegate`, one tile per object — hero + id/name + status chip + integration), toggled by two `QToolButton`s + a zoom `QSlider` (persisted view mode + tile size) in a `QStackedWidget`; both views build from one shared `_current_items()`/filter pipeline so search/catalog-filter/captured-only and selection stay in sync across the toggle + a **Catalog filter** selector (All / Messier / Caldwell …) + a **"Captured only"** checkbox (default off; composes with catalog + search) + **search** + captured/deep/total **stat row** (per-filter), hosting the shared `DetailPane`; `select_object`/`reload` work identically regardless of active view; per-object import flow; **right-click → "Fill in missing metadata"** (reference) + **"Enrich online"** (Simbad, on a worker; `OnlineLookupError`→dialog) + **"Remove from Library"** (`catalog.remove_library_entry`; confirm, non-destructive) + **"Pin as priority" / "Mute"** (`pins.set_state`, #3 — a ▲/▼ marker on the Object cell/tile via `_pin_marker`; `pins_changed`→shell lightweight reload); edit-lock |
| `pages/summary.py` | landing dashboard — **goal progress** (per active catalog), category progress, processing-queue snapshot, current integrations, **Priority targets** (`_priority_rows`: hand-edited `priorities.toml` **+ manually pinned** Library objects, muted excluded; ▲-marked pins clickable; **empty-state** caption when nothing pinned — #3); **first-run welcome** when nothing captured yet (a welcome card + "Import images…" CTA emitting `go_to_import`, replacing the empty tables; goals still show); object rows → `open_object` |
| `pages/goals.py` | **Goals** page (5d): manage tracked goals (bundled + custom) — the picker is grouped into collapsible **All-sky / Northern / Southern / Custom** sections (`_CollapsibleSection`, keyed off each goal's `hemisphere` from `goals.list_goals`), each catalog carrying a short description caption + a "Learn more ↗" source link (`seed/catalogs/*.toml` `hemisphere`/`source_url`); activate/deactivate checkboxes (→ `goals.set_active_goals`, emits `dirty`), **New custom goal…**/Edit/Delete (member identifiers resolved offline via `catalog.resolve_new_object`); per-active-goal **progress** line + a full **per-catalog membership table** (`_catalog_table`: Object · Name · **Captured** · **Deep stack**, a green check per column from `derived.totals_by_slug` status; rows clickable → `open_object`, height-capped so long catalogs scroll; **right-click a member → Pin/Mute** with a ▲/▼ marker, #3). Goal management lives here, not Preferences |
