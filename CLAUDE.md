...
| `pages/planning.py` | **Planning** pane (`feature/planning-profiles`) — session-planning home. A **location selector** (`planning_config.list_profiles`, sets `active_site_profile`), a **Priority targets** table (pins-only until the scorer lands; right-click Pin/Deprioritize → `pins_changed`), and a **Manage site profiles** collapsible hosting `SiteProfileEditor`. Persistent widgets (selector + editor built once, reloaded); rows → `open_object` |
| `site_profile_editor.py` | **site-profile editor** form (`feature/planning-profiles`) over `planning_config` writers: name/lat/lon/elev/timezone + Bortle/SQM + **Import** horizon `.hrz` (`import_horizon_mask`) + **Look up location…** (online `geocode`) + New/Save/Delete (default protected). Emits `saved`/`created`/`deleted`; the Planning page reloads the selector on each |
