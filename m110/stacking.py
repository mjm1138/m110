...
def build_ssf_solve(lights_name: str, p: Proposal,
    ...
    L = [
        ...
        "# Phase 1 of 3: calibrate, background-extract, plate solve.",
        "requires 1.4.0",
        "",
        f"cd {lights_name}",
...
def build_ssf_apply(lights_name: str, p: Proposal) -> str:
    """Phase 2: apply the registration the solve phase left in the .seq.

    A separate Siril invocation, which is the whole point — see `build_ssf_solve`.
    The sequence is re-read from disk, so this is reached whether phase 1 exited
    clean or aborted on a partial solve.
    ...
    L = [
        "# Generated by m110-stack — phase 2 of 3: apply registration.",
        "requires 1.4.0",
        "",
        "cd process",
    ]
    if p.get("compress"):
        L += [
            "# Re-stated because this is its own Siril process: the registered",
            "# sequence is by far the biggest thing this pipeline writes.",
            f"setcompress 1 -type=rice {p.get('compress_quant')}",
        ]
    L.append("")
...
def build_ssf_stack(lights_name: str, p: Proposal, drop: list[int]) -> str:
    """Phase 3: stack the registered sequence, excluding any empty frames.

    Separate from the phases before it so the degenerate-frame check can run in
    between, and so stack settings can be re-tried without repeating registration
    — on a large mosaic the stack is nearly all of the wall clock, but
    registration is still minutes you should not have to pay twice.
    """
    _, seq = seq_names(lights_name, p)
    L = [
        "# Generated by m110-stack — phase 3 of 3: stack.",
        "requires 1.4.0",
        "",
        "cd process",
...
# "Image bkg_pp_lights_00146 did not solve" — the frame's 1-based position in
# the sequence is the zero-padded tail of its name, and `link` ingests lights/ in
# the same sorted order `read_frames` walks it, so that number indexes straight
# into the frame list. The name prefix can itself contain digits, hence the lazy
# \S*? in front: only the run of digits immediately before " did not solve" can
# satisfy the pattern.
_DID_NOT_SOLVE = re.compile(r"Image\s+\S*?(\d+)\s+did not solve")
_SOLVE_TALLY = re.compile(
    r"(\d+)\s+images successfully platesolved out of\s+(\d+)\s+included")


@dataclass
class SolveOutcome:
    """What phase 1's log says about how many frames actually plate-solved.

    `solved is None` means the log never got as far as saying — the run broke
    somewhere else (a missing catalogue, a failed calibrate) and the exit code is
    the only thing worth reporting. That is a genuinely different situation from
    a solve that partly worked, and conflating them is what produced
    "Registration failed (exit 1)" for a run with 123 perfectly good frames.
    """
    ...
    @property
    def known(self) -> bool:
        return self.solved is not None and bool(self.included)

    @property
    def complete(self) -> bool:
        return self.known and self.solved >= self.included

    @property
    def fraction(self) -> float:
        return (self.solved / self.included) if self.known else 0.0


def parse_solve_log(text: str) -> SolveOutcome:
    """Read the solve tally and the failed frames out of a Siril log."""
    o = SolveOutcome()
    m = None
    for m in _SOLVE_TALLY.finditer(text):      # last tally wins
        pass
    if m:
        o.solved, o.included = int(m.group(1)), int(m.group(2))
    o.failed = [int(n) for n in _DID_NOT_SOLVE.findall(text)]
    return o


def failures_by_night(failed: list[int], frames: list[Frame],
                      excluded: list[int] | None = None) -> dict[str, tuple[int, int]]:
    """night -> (frames that failed to solve, frames offered to the solver).

    Totals count only the frames the solver was actually given, so a night the
    user already dropped with --exclude-night does not appear as a suspiciously
    clean 0-of-0. Indices are 1-based positions in the full sorted lights list,
    the same convention `select_frames` produces.
    """
    ...
    for i, fr in enumerate(frames, start=1):
        if i in skip:
            continue
        night = fr.date or "unknown"
        tally = out.setdefault(night, [0, 0])
        tally[1] += 1
        if i in bad:
            tally[0] += 1
...
#: A night is only worth offering as an --exclude-night when dropping it removes
#: more failures than it costs good frames — i.e. *more* than half of it failed.
#: Strictly more: an even split is a coin toss, not a bad session.
BAD_NIGHT_FRACTION = 0.5

#: ...and when there is enough of it to be a session rather than a coincidence.
#: Two frames out of two clear any fraction bar trivially; three failures is the
#: least that can distinguish "this night went wrong" from "these frames did".
BAD_NIGHT_MIN_FAILURES = 3


def format_solve_report(o: SolveOutcome, by_night: dict[str, tuple[int, int]],
                        min_solved: float, continuing: bool) -> str:
    """The partial-solve diagnosis, as the text the CLI prints.

    Names the nights because that is the actionable part: solve failures cluster
    by session — bad seeing, cloud, a bumped focuser — far more often than they
    scatter, and a named night is one --exclude-night away from a clean re-run.
    Whether they cluster is itself the diagnosis: concentrated on a night or two
    it is the sky, spread evenly across every night it is the setup, and those
    two want opposite fixes.
    """
    out = [""]
    pct = 100 * o.fraction
    out.append(f"  Plate solving solved {o.solved} of {o.included} frames ({pct:.0f}%).")

    culprits = [d for d, (bad, tot) in by_night.items()
                if tot and bad >= BAD_NIGHT_MIN_FAILURES
                and bad / tot > BAD_NIGHT_FRACTION]
    if len(by_night) > 1:
        out.append("  Failures by night:")
        for d, (bad, tot) in by_night.items():
            share = f"{100 * bad / tot:3.0f}%" if tot else "  - "
            mark = "  <--" if d in culprits else ""
            out.append(f"      {d}   {bad:4d} of {tot:4d}  {share}{mark}")
    drops = " ".join(f"--exclude-night {d}" for d in culprits)

    if continuing:
        out.append(f"  Continuing with the {o.solved} solved frames; the rest are "
                   "excluded from")
        out.append("  registration and from the stack.")
        if culprits:
            out.append("  To drop the bad night(s) outright instead and re-run:")
            out.append(f"      {drops}")
    else:
        out.append(f"  That is below the {100 * min_solved:.0f}% needed to carry on, "
                   "so nothing was registered or")
        out.append("  stacked.")
        if culprits:
            out.append("  The failures are concentrated, so drop the night(s) and "
                       "re-run:")
            out.append(f"      {drops}")
        else:
            out.append("  The failures are spread across the whole set rather than "
                       "one session, which")
            out.append("  points at the setup rather than the sky — check that the "
                       "local Gaia")
            out.append("  catalogue covers this field, and that the frames' focal "
                       "length and pixel")
            out.append("  size are right.")
        if o.solved:
            out.append(f"  To stack the {o.solved} that did solve, lower the bar: "
                       f"--min-solved {max(1, int(pct))}")
    out.append("")
    return "\n".join(out)
...
class StackPlan:
    ...
    def as_json(self) -> dict:
        ...
        return {
            ...
            # All three phases, named for what they are. A single "script" key
            # was a half-truth, and so was pairing "register" with "stack": the
            # solve is its own Siril run precisely so a partial plate solve
            # cannot take registration down with it.
            "solve_script": self.solve_ssf,
...
def build_plan(directory, ov: Overrides | None = None, *, siril: str | None = None,
    ...
    return StackPlan(
        ...
        solve_ssf=build_ssf_solve(lights.name, p, dropped),
        register_ssf=build_ssf_apply(lights.name, p),
        stack_ssf=build_ssf_stack(lights.name, p,
                                  find_degenerate(working / "process",
                                                  seq_names(lights.name, p)[1])),
        frames_total=len(all_frames), dropped=dropped, frames=all_frames,
    )
...
def main() -> int:
    ...
    ap.add_argument("--ssf", type=Path, metavar="PATH",
                    help="write the generated scripts beside PATH, one per phase "
                         "(PATH.solve.ssf, PATH.register.ssf, PATH.stack.ssf). "
                         "They are separate Siril runs by design — concatenating "
                         "them reinstates the partial-solve abort.")
    ...
    ap.add_argument("--min-solved", type=float, default=25.0, metavar="PCT",
                    help="minimum %% of frames that must plate-solve before the "
                         ...
                         "Solve failures usually cluster on one bad night, and "
                         "the solved subset still stacks; a rate below this is "
                         "more likely a broken setup than a bad night, so the "
    ...
    ssf_solve, ssf_reg, ssf_stack = (plan.solve_ssf, plan.register_ssf,
                                     plan.stack_ssf)
    ...
    if a.ssf:
        # One file per phase, because one file is exactly the bug: run these as a
        # single script and a partial plate solve aborts it before the frames
        # that did solve are ever registered.
        print("")
        for phase, text in (("solve", ssf_solve), ("register", ssf_reg),
                            ("stack", ssf_stack)):
            ext = a.ssf.suffix or ".ssf"
            dest = a.ssf.with_name(f"{a.ssf.stem}.{phase}{ext}")
    ...
    if not a.run:
        print("\n--- phase 1: solve " + "-" * 46)
        print(ssf_solve)
        print("--- phase 2: register " + "-" * 43)
        print(ssf_reg)
        print("--- phase 3: stack " + "-" * 46)
        print(ssf_stack)
        print("-" * 65)
    ...
    if a.restack:
        ...
        if outcome.known and not outcome.complete:
            min_solved = max(0.0, min(1.0, a.min_solved / 100.0))
            carry_on = outcome.solved > 0 and outcome.fraction >= min_solved
            print(format_solve_report(
                outcome,
                failures_by_night(outcome.failed, plan.frames, dropped),
                min_solved, carry_on), flush=True)
            if not carry_on:
                print(f"Log: {log_path}\n")
                return 1
            # Siril fails the *script* on a partial solve, so phase 1's log is
            # full of "...failed" lines describing a situation we have just
            # reported properly and chosen to continue past. Repeating them at
            # the end, next to a successful stack, would only contradict it.
            errors = []
        elif rc != 0:
            print(f"\nPlate solving failed (exit {rc}). Log: {log_path}\n")
            return 1

        # ---- phase 2: apply the registration -----------------------------
        ...
        if rc != 0:
            print(f"\nRegistration failed (exit {rc}). "
                  f"Log: {log_path.with_name('siril_stack_register.log')}\n")
            return 1
