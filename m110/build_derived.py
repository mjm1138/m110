...
def build_processing(totals: dict, overrides: dict | None,
    ...
    """
    from . import processing    # Qt-free engine module; the per-workflow ready flag
    ...
    for fname, t in by_folder.items():
        ...
        out[fname] = {
            ...
            # Finished output sitting in a workflow sandbox, not yet imported
            # (#beta): drives the Processing page's "Ready to import" group.
            # `ready_for_import` stays a boolean so every existing reader keeps
            # working; `ready_workflows` is what answers "which tool?", which a
            # single flag cannot once there is more than one (ROADMAP 14b).
            "ready_for_import": bool(processing.workflows_with_output(fname)),
            "ready_workflows": [w.id for w in
                                processing.workflows_with_output(fname)],
