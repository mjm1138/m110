...
def list_snapshots(destination: Path, store_name: str | None = None) -> list[SnapshotInfo]:
    ...
    *names* parse as timestamps, and `objects/`/`snapshots/`/`latest/` never will.
    ...
    full restore access to everything they already had."""
    snaps = (mirrored.list_snapshots(destination, store_name) +
             pooled.list_snapshots(destination, store_name))
...
def apply_retention(destination: Path, *, keep: int | None = None,
    ...
    if dropped:
        ...
        if pooled_ts:
            pooled.forget_snapshots(destination, pooled_ts, store_name)
    ...
    if gc and _sweep_due(destination, store_name, bool(dropped)):
        swept = sweep_objects(destination, store_name).get("swept", 0)

    return {"pruned": len(dropped), "kept": len(snaps) - len(dropped), "swept": swept}


def _sweep_due(destination: Path, store_name: str | None, pruned: bool) -> bool:
    ...
    if not (store_backup_root(destination, store_name) / "objects").is_dir():
        return False
    if pruned:
        return True
...
def sweep_objects(destination: Path, store_name: str | None = None, *,
    ...
    for snap in pooled.list_snapshots(destination, store_name):
        for meta in pooled.snapshot_files(snap.path).values():
