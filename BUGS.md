...
- [x] Import: a loose finished render (`<obj>/<obj>_…_processed.png`) lands in the holding area instead of being recognized. *Root cause — the layout classifier had no rule for a loose **finished raster**: a processed `.png/.jpg/.tif` is only auto-recognized inside a `finished/`/`Finished Images/` folder; raw-fits only fires on FITS, so a bare `*_processed.png` in an object-named folder fell through to holding. Done — new **finished-render** recognizer claims a loose `*_processed/final/finished` raster → the object's `finished/` tier (object from the folder name); non-finished rasters are not over-claimed (still held). Tests: `test_ingest.test_loose_finished_raster_routes_to_finished` + `…_still_held`.*
...
- [ ] **#27**: **Publishing follow-ups** (ROADMAP item 8; 8a landed 2026-06-29 — local static-site export). Deferred targets/refinements that build on the `publish/` engine + registry: (a) a **GitHub Pages deploy** target (git/`ghp-import` push to `gh-pages`, repo from settings, image-cache preservation like the Astronomy CI flow) — the registered-disabled `github-pages` placeholder; (b) **Netlify / S3·CloudFront / WordPress·Ghost** targets; (c) **per-list (goal) publish flags** (8a ships per-object `publish` + per-journal `private` only); (d) **cross-publish image-cache reuse** (8a re-generates web thumb/full derivatives each run into the output `img/` — content-hash cached within a run/dir, but not shared across output dirs or pruned — the publish analogue of #14); (e) optional **auto-publish on refresh**. The publisher registry (`publish.PUBLISHERS`) + `PublishOptions` are the stable seams; each target is an adapter.
