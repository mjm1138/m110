...
<data_root>/
  ...
  .m110_internal_data/              hidden machine state (README: "don't touch")
    ...
    pins.toml                       manual Pin/Mute overrides ([pins] slug="pin"|"mute"; lazily created,
...
| Pins | `.m110_internal_data/pins.toml` | TOML (`[pins]` slug→`"pin"`\|`"mute"`) | Manual Pin/Mute priority overrides (#3); app-written (`pins.set_state`); lazily created, additive (no `.store_version` bump), **survives derived regeneration** | **Mutable** — user-owned | Persistent | Never auto-deleted |
